...
journal_modify_block_end(journal *jnl, struct buf *bp, void (*func)(buf_t bp, void *arg), void *arg)
{
	...
	if (i >= blhdr->num_blocks) {
		...
		/*
		 * Newer kernels track physical metadata writes separately; older
		 * (10.15-era) kernels only provide logical write accounting.
		 */
#if defined(TASK_PHYSICAL_WRITE_METADATA) && defined(TASK_BALANCE_CREDIT)
