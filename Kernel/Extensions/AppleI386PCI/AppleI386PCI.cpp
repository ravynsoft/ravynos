...
static inline uint32_t inl(i386_ioport_t port)
{
    uint32_t data;
...
static inline uint16_t inw(i386_ioport_t port)
{
    uint16_t data;
    __asm__ volatile(".byte 0x66; inl %%dx, %%eax"
        : "=a"(data)
        : "dN"(port));
    return data;
}
...
static inline void outl(i386_ioport_t port, uint32_t data)
{
...
static inline void outw(i386_ioport_t port, uint16_t data)
{
    __asm__ volatile(".byte 0x66; outl %%eax, %%dx"
        :
        : "a"(data), "dN"(port));
}

static inline void outb(i386_ioport_t port, uint8_t data)
{
