 ...
 *
 * Bring-up AHCI register definitions for ravynOS.
 * Based on Serial ATA AHCI 1.3.1 specification.
 */
...
#define AHCI_CMD_TABLE_PRDT_OFFSET  (AHCI_CMD_TABLE_CFIS_SIZE + AHCI_CMD_TABLE_ACMD_SIZE + AHCI_CMD_TABLE_RSVD_SIZE)
/* We use 1 PRD per command during bring-up */
#define AHCI_CMD_TABLE_SIZE         (AHCI_CMD_TABLE_PRDT_OFFSET + sizeof(AHCIPrd))
...
/* Command List: 32 × 32-byte headers = 1024 bytes (1 KiB, 1K-aligned) */
...
#define AHCI_CLB_OFFSET     0
#define AHCI_FIS_OFFSET     (AHCI_CLB_SIZE)
#define AHCI_CTBL_OFFSET    (AHCI_CLB_SIZE + AHCI_FIS_SIZE)
#define AHCI_DMA_OFFSET     (AHCI_CLB_OFFSET + 4096)  /* 4K-aligned */
