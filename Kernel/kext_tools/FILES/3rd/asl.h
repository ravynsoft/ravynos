 ...
 */
const char *asl_key(asl_object_t msg, uint32_t n);
 ...
 */
int asl_log(asl_object_t client, asl_object_t msg, int level, const char *format, ...) __printflike(4, 5) __not_tail_called;
 ...
 * @result Returns 0 for success, non-zero for failure.
 */
int asl_log_message(int level, const char *format, ...) __printflike(2, 3) __not_tail_called;
 ...
 *
 */
int asl_vlog(asl_object_t obj, asl_object_t msg, int level, const char *format, va_list ap) __printflike(4, 0) __not_tail_called;
 ...
 *    types of input object.
 */
int asl_send(asl_object_t obj, asl_object_t msg) __not_tail_called;
 ...
 */
void asl_append(asl_object_t obj, asl_object_t obj_to_add) __not_tail_called;
 ...
 *    (input) An object of type ASL_TYPE_MSG, ASL_TYPE_QUERY or type ASL_TYPE_LIST.
 */
void asl_prepend(asl_object_t obj, asl_object_t obj_to_add);
 ...
 */
size_t asl_count(asl_object_t obj);
 ...
 */
asl_object_t asl_get_index(asl_object_t list, size_t index);
 ...
 *    (input) An object of type ASL_TYPE_LIST.
 */
void asl_remove_index(asl_object_t list, size_t index);
 ...
 */
asl_object_t asl_open_from_file(int descriptor, const char *ident, const char *facility);
 ...
 */
char *asl_format(asl_object_t msg, const char *msg_fmt, const char *time_fmt, uint32_t text_encoding);
 ...
 */
char *asl_encode_buffer(const char *buf, size_t len);
 ...
 * @result Returns 0 on success, non-zero on failure.
 */
int asl_decode_buffer(const char *in, char **buf, size_t *len);
 ...
 *    Returns NULL when there are no more messages or if obj is not a type that holds messages.
 */
asl_object_t asl_prev(asl_object_t obj);
 ...
 */
void asl_reset_iteration(asl_object_t obj, size_t position);
