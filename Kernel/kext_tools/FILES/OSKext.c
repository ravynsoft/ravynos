...
/*
 * This is a modified version of OSKext.c which is only used for the ravynOS
 * host toolchain as part of building the kextcache tool. This is normally
 * part of the IOKit framework.
 */
...
static Boolean                         __sOSKextUsesCaches                  = FALSE; //TRUE;
static Boolean                         __sOSKextStrictRecordingByLastOpened = FALSE;
static Boolean                         __sOSKextStrictAuthentication        = FALSE;
static OSKextAuthFnPtr                 __sOSKextAuthenticationFunction      = NULL; // _OSKextBasicFilesystemAuthentication;
...
{
#if HOST_BUILD
    static const NXArchInfo _ArchX86 = {
        "X86_64", CPU_TYPE_X86_64, CPU_SUBTYPE_X86_64_ALL,
        __LITTLE_ENDIAN__, "Intel 64-bit"
    };
    return &_ArchX86;
#else
                                        ...
                                        KXLDContext *    kxldContext)
{
    ...
    linkAddressContext.kernelLoadAddress = 0; //NULL;
    ...
    CFArrayRef personalityNames)
{
    ...
    if (!count) {
        ...
        context.kext               = aKext;
        context.personalitiesArray = (CFMutableArrayRef)result;
        context.personalitiesDict  = NULL; /* We want an array, not a dict */
...
__unused static void __OSKextShowPLKInfo(plkInfo *info)
{
    ...
    for (enum enumSegIdx segIdx = SEG_IDX_TEXT; segIdx < SEG_IDX_COUNT; segIdx++) {
        ...
        OSKextLog(NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
                  "plk_%s: vmaddr %p vmsize %llu fileoff %llu next_fileoff %llu filesize %llu",
