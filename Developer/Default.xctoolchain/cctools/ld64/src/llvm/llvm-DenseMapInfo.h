...
struct DenseMapInfo<T*> {
  ...
  static unsigned getHashValue(const T *PtrVal) {
    return std::hash<uintptr_t>{}((uintptr_t)PtrVal);
  }
  static bool isEqual(const T *LHS, const T *RHS) { return LHS == RHS; }
};

// Provide customizable DenseMapInfo for custom hash/eq op of pointer keys.
template<typename T, typename Op>
struct PointerContentDenseMapInfo {
  static inline T* getEmptyKey() {
    uintptr_t Val = static_cast<uintptr_t>(-1);
    return reinterpret_cast<T*>(Val);
  }
...
template<> struct DenseMapInfo<std::string_view> {
  static inline std::string_view getEmptyKey() { 
    return std::string_view(DenseMapInfo<const char*>::getEmptyKey(), 0);
  }
  static inline std::string_view getTombstoneKey() { 
    return std::string_view(DenseMapInfo<const char*>::getTombstoneKey(), 0);
  }
  static unsigned getHashValue(const std::string_view &Val) { 
    return std::hash<std::string_view>{}(Val);
  }
  static bool isEqual(const std::string_view& LHS, const std::string_view& RHS) {
    if ( LHS.size() != RHS.size() )
      return false;
    if ( LHS.data() == RHS.data() )
      return true;

    if (LHS.data() == getEmptyKey().data() || RHS.data() == getEmptyKey().data())
      return false;
    if (LHS.data() == getTombstoneKey().data() || RHS.data() == getTombstoneKey().data())
      return false;
