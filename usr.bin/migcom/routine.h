...
#define akeSubCount     (14)    /* a array of array case: subordinate arrays
                                 * count */
#define akeImplicit     (15)    /* an implicit argument, from the trailer */
#define akeSecToken     (16)    /* an argument from the trailer: the security
                                 * token */
#define akeAuditToken   (17)    /* an argument from the trailer: the audit
                                 * token */
#define akeContextToken (18)    /* an argument from the trailer: the context
                                 * token */
...
#define akbReplyInit	(0x00080000)    /* reply value doesn't come from
                                         * target routine */
...
#define akbSendKPD 	(0x08000000)    /* the arg is sent in the Kernel
                                         * Processed Data section of the
                                         * Request message */
#define akbReturnKPD 	(0x10000000)    /* the arg is sent in the Kernel
                                         * Processed Data section of the
                                         * Reply message */
...
typedef struct argument {
    ...
    mach_msg_descriptor_type_t argKPD_Type;     /* KPD type: port, ool,
                                                 * port+ool */
    void            (*argKPD_Template) ();      /* KPD discipline for static
                                                 * templates */
    void            (*argKPD_Init) ();  /* KPD discipline for initializing */
    void            (*argKPD_Pack) ();  /* KPD discipline for packing */
    void            (*argKPD_Extract) ();       /* KPD discipline for
                                                 * extracting */
    void            (*argKPD_TypeCheck) ();     /* KPD discipline for type
                                                 * checking */
    ...
    struct argument *argSubCount;       /* our sub-count arg, if present
                                         * (variable subordinate arrays) */
    ...
    struct argument *argSameCount;      /* the arg to take the count from, if
                                         * present */
    struct argument *argParent; /* in a count or poly arg, the base arg */
    u_int           argMultiplier;      /* for Count argument: parent is a
                                         * multiple of a basic IPC type.
                                         * Argument must be multiplied by
                                         * Multiplier to get IPC
                                         * number-of-elements. */
    ...
    boolean_t       argTempOnStack;     /* A temporary for the
                                         * short-circuiting code when
                                         * -maxonstack is used. */
}               argument_t;
...
typedef struct routine {
    ...
    u_int           rtNumRequestVar;    /* number of variable/inline args in
                                         * request */
    u_int           rtNumReplyVar;      /* number of variable/inline args in
                                         * reply */

    u_int           rtMaxRequestPos;    /* maximum of argRequestPos */
    u_int           rtMaxReplyPos;      /* maximum of argReplyPos */

    u_int           rtRequestKPDs;      /* number of Kernel Processed Data
                                         * entries */
    u_int           rtReplyKPDs;        /* number of Kernel Processed Data
                                         * entries */
    u_int           rtOverwrite;        /* number of Overwrite entries */
    u_int           rtOverwriteKPDs;    /* number of entries in the Overwrite
                                         * template */

    boolean_t       rtNoReplyArgs;      /* if so, no reply message arguments
                                         * beyond what the server dispatch
                                         * routine inserts */

    boolean_t       rtRequestFits;      /* Request fits within onstack limit */
    boolean_t       rtReplyFits;        /* Reply fits within onstack limit */
    boolean_t       rtRequestUsedLimit; /* User type limit used in deciding
                                         * whether request fits within
                                         * onstack limit */
    boolean_t       rtReplyUsedLimit;   /* User type limit used in deciding
                                         * whether reply fits within onstack
                                         * limit */
    u_int           rtRequestSizeKnown; /* Max size of known portion of
                                         * request */
    u_int           rtReplySizeKnown;   /* Max size of known portion of
                                         * request */
    ...
    argument_t     *rtRetCArg;  /* the Routine has this argument tagged as
                                 * RetCode */
    argument_t     *rtRequestPort;      /* always non-NULL, defaults to first
                                         * arg */
    argument_t     *rtReplyPort;        /* always non-NULL, defaults to
                                         * Mig-supplied */
    ...
    u_int           rtCountOolPortsIn;  /* how many out_of-line Ports are
                                         * sent */
    u_int           rtCountOolIn;       /* how many bytes out_of-line are
                                         * sent */

    u_int           rtCountPortsOut;    /* how many in-line Ports are rcv'd */
    u_int           rtCountOolPortsOut; /* how many out_of-line Ports are
                                         * rcv'd */
    u_int           rtCountOolOut;      /* how many bytes out_of-line are
                                         * rcv'd */

    u_int           rtTempBytesOnStack; /* A temporary for the
                                         * short-circuiting code when
                                         * -maxonstack is used. */
