...
safe_keyname (SCREEN *sp, int c)
{
	...
	if (c == -1) {
		...
		if (result == NULL && (c >= 0 && c < SIZEOF_TABLE)) {
			if (MyTable == NULL)
				MyTable = typeCalloc(char *, SIZEOF_TABLE);

			if (MyTable != NULL) {
				int m_prefix = (sp == NULL || sp->_use_meta);
...
#define P_LIMIT (sizeof(name) - (size_t) (p - name))
		...
		} else if (result == NULL && HasTerminal(sp)) {
			int j, k;
			char * bound;
			...
			for (j = 0; (bound = NCURSES_SP_NAME(keybound)(NCURSES_SP_ARGx c, j)) != NULL; ++j) {
				for(k = STRCOUNT; k < (int) NUM_STRINGS(tp);  k++) {
					if (tp->Strings[k] != NULL && !strcmp(bound, tp->Strings[k])) {
